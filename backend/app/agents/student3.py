"""Deterministic Student 3 agents with additive ML-backed segmentation support.

These modules derive recommendations from the submitted description, the industry model output,
and the funding rubric. When a trained segmentation artifact is available, the customer segment
module can attach a clustering-derived segment; otherwise it returns a clear degraded non-ML
fallback so the system never pretends to have a live trained model.
"""
...
def growth_strategy(segment: dict, actions: list[dict], industry: dict | None) -> list[dict]:
    ...
    items = [
        ...
        GrowthItem(area="acquisition", recommendation=acquisition, rationale="Channel selection requires a trained customer segment or direct channel evidence.", dependency="A testable value proposition.", assumptions=["The inferred industry ({domain}) is directionally useful."]),
